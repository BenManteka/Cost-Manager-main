...
app.use(pinoHttp({
  ...
  customProps: (req) => ({ route: req.originalUrl, method: req.method })
}));

// Connect to MongoDB using the connection string from environment variables
// In test mode donwt connect to MongoDB -> use the connection from tests/test-db.setup.js
...
app.use("/api", aboutRoutes); // Route for team/about information
 ...
 // define condition to avoid using the PORT if it's a test
if (process.env.NODE_ENV !== 'test') { 
